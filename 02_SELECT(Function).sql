...
ORDER BY 
    FLOOR((SYSDATE - TO_DATE('19'||SUBSTR(PROFESSOR_SSN, 1, 6), 'YYYY-MM-DD'))/365)
    , PROFESSOR_NAME;
...
--     SUBSTR(TO_CHAR(TO_DATE('19'||SUBSTR(STUDENT_SSN, 1, 6), 'YYYYMMDD'), 'YYYYMMDD'),1 ,4) AS "차"
--     ,SUBSTR(TO_CHAR(ENTRANCE_DATE, 'YYYYMMDD'), 1, 4) - SUBSTR(TO_CHAR(TO_DATE('19'||SUBSTR(STUDENT_SSN, 1, 6), 'YYYYMMDD'), 'YYYYMMDD'), 1, 4) AS "1"
-- FROM
--     TB_STUDENT
...
--'DAY': 금요일 'DY': 금 'D': 6
SELECT
    TO_CHAR(TO_DATE('20201225', 'YYYYMMDD'), 'DAY') DAY
    , TO_CHAR(TO_DATE('20201225', 'YYYYMMDD'), 'DY') DY
    , TO_CHAR(TO_DATE('20201225', 'YYYYMMDD'), 'D') D
FROM
    DUAL;
...
/*
    where 절
        ABSENCE_YN = 'Y' 로 쿼리 날리면 휴학생이 없는 학과는 출력이 안됨..!
*/
SELECT
    DEPARTMENT_NO AS "학과 번호"
    , COUNT(DECODE(ABSENCE_YN, 'Y', 1, NULL)) AS "휴학생 수"
FROM
    TB_STUDENT
GROUP BY DEPARTMENT_NO
ORDER BY DEPARTMENT_NO;
